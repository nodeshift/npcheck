...
module.exports = async (module, _, options) => {
  ...
  const commitHistory = await fetchGithub(`/repos/${githubTarget}/commits`, options.githubToken);
  ...
  const repoStatuses = await fetchGithub(`/repos/${githubTarget}/commits/${commitHash}/statuses`, options.githubToken);
