...
module.exports = async (pkg, config, options) => {
  ...
  npmCommand = npmCommand.concat(` ${npmInfo.name}`);
  ...
  const moduleDependencies = Object.entries(packageLicenses).filter(([pkg]) => !pkg.includes(npmInfo.name));
  ...
  for (const [key, value] of moduleDependencies) {
    ...
    const licensesSpecific = config.licenses.rules[npmInfo.name]?.allow || [];
    ...
    const licenseOverrides = config.licenses.rules[npmInfo.name]?.override || [];
    ...
    if (licenseForcePass) {
      ...
      stats.logs.push({
        type: 'warning',
        message: `The module "${npmInfo.name}" depends on the "${key}" package which is under the yet undetermined license "${value.licenses}". (Manual review needed)`
      });
      warn(output);
      continue;
    }
    ...
    stats.logs.push({
      type: 'error',
      message: `The module "${npmInfo.name}" depends on the "${key}" package which is under the non-acceptable license "${value.licenses}".`
    });
    fail(output);
  }
